    ...
    $.getJSON(BaseUrl + "OfficersVacanciesMasterAPI/GetT_OfficersVacanciesMaster",
        function (Data) {
            $.each(Data, function (key, val) {
                if (val.TypOfVacancy === "3") {
                    var LDate = val.LastDate.split('T');
                $("#GovtCompList").append("<table class='table table-bordered table-striped table-responsive table-hover small  m-2 shadow w-100'><tr><th>Company Name </th><td>" + val.CompanyName + "</td></tr><tr><th>No of Post </th><td>" + val.NoOfPost
